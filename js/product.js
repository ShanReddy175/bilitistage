...
$('.watch-video').on('click',()=>{
  $('.play-video-div').addClass('active');
})

$('.youtubeCloseIcon').on('click',()=>{
  $('.play-video-div').removeClass('active');

  $('iframe#videoIframe').attr('src', $('iframe#videoIframe').attr('src'));
})
...
if(isMobile()){
  var productImgDiv = document.querySelector('.product-360deg-img'),
      productDeatils = document.querySelector('.product-360deg-details'),
      productH2 = document.querySelector('.product-360deg h2'),
      product360deg = document.querySelector('.product-360deg-btn'),
      productColors = document.querySelector('.product-different-colors');

      productImgDiv.style.height = `calc(100vh - ${productDeatils.clientHeight + productH2.clientHeight}px)`;
      product360deg.style.bottom = `calc(2% + ${productDeatils.clientHeight}px)`;
      productColors.style.bottom = `calc(10% + ${productDeatils.clientHeight}px)`;
}


/* gallery Image open in full width */

$('.gallery-closeIcon').on('click',()=>{
  $('.gallery-img-popup').removeClass('activated');
  $('.gallery-popup-img').attr('src','');
})


const galleryImgs = document.querySelectorAll('.product-gallery-section img');

galleryImgs.forEach((gImg)=>{
  gImg.addEventListener('click',()=>{
    $('.gallery-img-popup').addClass('activated');
    console.log(gImg.getAttribute('alt'))
    $('.gallery-popup-img').attr('src',`images/product/gallery/full/${gImg.getAttribute('alt')}.png`)
  })
})